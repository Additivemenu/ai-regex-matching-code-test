    ...
    try: 
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Given the natural language query: '{query}', return in JSON format: \n field 'regex_pattern': the regex expression that user want to match - if you cannot find it give it a null value,  \n field 'replacement': the replacement value that user want to replace the matching regex pattern - if you cannot find it give it a null value,\n field 'column_name': the column name that user want to perform operation - if you cannot find it give it a null value"},
                # {"role": "user", "content": f"Given the natural language query: '{query}', return in JSON format: \n field 'regex_pattern': the regex expression that user want to match - if you cannot find it give it a null value,  \n field 'replacement': the replacement value that user want to replace the matching regex pattern - if you cannot find it give it a null value,\n field 'column_name': the column name that user want to perform operation - if you cannot find it give it a null value"}
            ]
        )
