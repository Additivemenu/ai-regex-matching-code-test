...
def handle_data_transformation(table_data, user_query):
    ...
    if LLM_res.column_name not in table_data_headers:
        raise HttpError(404, f"Column name {LLM_res.column_name}not found in table data, please check your spelling (note column name should be case sensitive) and type in a valid table header name")
